...
const Navbar = () => {
  ...
  return (
    <nav className="px-6 md:px-[8rem] 2xl:px-[12rem] w-screen py-6 flex justify-between">
      <h1 className="text-white font-montserrat font-bold text-xl md:text-2xl">
        ARNAS.
      </h1>
      <ul className="hidden font-poppins font-medium text-white md:flex">
        {navMenu.map((menu, index) => (
          <li
            key={index}
            className={index === navMenu.length - 1 ? "mr-0" : "mr-8"}
          >
            {menu.name}
          </li>
        ))}
